...
for message in consumer:
	...
	if len(dataset) < MIN_TRAIN:
		...
		stream.append(closing_stock)
		dataset.append(({'x': stream[-2]}, stream[-1]))
		river_bagging_trees.learn_one({'x': stream[-2]}, stream[-1])
		river_hoeffding = river_hoeffding.learn_one({'x': X_t}, y_t)
		...
		if (i > MIN_TRAIN) and (i % TRAIN_EVERY == 0):
			...
			plt.plot(rbt_rmse, color='red', label='Bagging regressor')
			plt.plot(hfdg_rmse, color='blue', label='Hoeffding tree regressor')
			if not is_plotted:
				plt.legend()
