...
for message in consumer:
	...
	if len(dataset) < MIN_TRAIN:
		...
		if (i > MIN_TRAIN) and (i % TRAIN_EVERY == 0):
			...
			plt.plot(lm_rmse, color='magenta', label='Linear model')
			plt.plot(river_lm_rmse, color='cyan', label='River linear model')
			plt.plot(rf_rmse, color='green', label='Random forest')
			plt.plot(rbt_rmse, color='red', label='River Bagging regressor')
			plt.plot(hfdg_rmse, color='blue', label='River Hoeffding tree regressor')
			if not is_plotted:
				plt.legend()
